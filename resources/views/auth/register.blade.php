<x-guest-layout>
    <h1 class="text-lg font-semibold text-gray-800 mb-4">{{ __('auth_ui.register_title') }}</h1>

    <form id="registerForm" method="POST" action="{{ route('register') }}" novalidate>
        @csrf

        <div>
            <x-input-label for="name" :value="__('auth_ui.field_name')" />
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" maxlength="80" />
            <p data-error-for="name" class="mt-1 text-xs text-red-600 hidden"></p>
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <x-primary-button class="ms-4">{{ __('auth_ui.register_button') }}</x-primary-button>
        </div>
    </form>

    {{-- Client-side validation: runs BEFORE the server, all messages from lang/en/auth_ui.php --}}
    <script>
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!form.name.value.trim())                    { setError('name', MSG.name_required); ok = false; }
